...
ds t1 t2 = case (t1, t2) of
  ...
  (Comb _ _, Var t4) -> if t4 == VarName "_" then Nothing else Just (t2, t1)
  (Var t3, Comb _ _) -> if t3 == VarName "_" then Nothing else Just (t1, t2)
...
unMaybe :: a -> Maybe a -> a
unMaybe a ma = case ma of
  Just x -> x
  Nothing -> a

-- unify algorithm
unify :: Term -> Term -> Maybe Subst
unify t1 t2 = unifyGen t1 t2 empty

--  | null (substToList (unifyGen t1 t2 empty)) = Nothing
--  | otherwise = Just (unifyGen t1 t2 empty)

-- helps with recursion of unify algorithm
unifyGen :: Term -> Term -> Subst -> Maybe Subst
unifyGen t1 t2 mgu = case ds (apply mgu t1) (apply mgu t2) of -- hier werden ganze terme verglichen, nicht die elemente des ds!!!
  Just (Var t6, Var t5) -> if t6 == t5 then Nothing else unifyGen t1 t2 (single t6 (Var t5) `compose` mgu)
  Just (Var t3, Comb cName termList1) -> if t3 `elem` concatMap allVars termList1 then Nothing else unifyGen t1 t2 (single t3 (Comb cName termList1) `compose` mgu)
